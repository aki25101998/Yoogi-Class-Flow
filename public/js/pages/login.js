// Login page
import { signInWithGoogle } from '../auth.js';
import { showToast } from '../components/toast.js';

/**
 * Render the login page
 * @param {HTMLElement} container - #app element (full page)
 */
export function renderLoginPage(container) {
  container.innerHTML = `
    <div class="login-page">
      <div class="login-card">
        <div class="login-logo">
          <span class="material-icons-round">sports_martial_arts</span>
        </div>
        <h1 class="login-title">Quản Lý Chấm Công</h1>
        <p class="login-subtitle">Hệ thống điểm danh và tính lương cho Huấn luyện viên</p>
        
        <button class="btn btn-google btn-lg w-full" id="btnGoogleLogin">
          <svg width="20" height="20" viewBox="0 0 48 48">
            <path fill="#EA4335" d="M24 9.5c3.54 0 6.71 1.22 9.21 3.6l6.85-6.85C35.9 2.38 30.47 0 24 0 14.62 0 6.51 5.38 2.56 13.22l7.98 6.19C12.43 13.72 17.74 9.5 24 9.5z"/>
            <path fill="#4285F4" d="M46.98 24.55c0-1.57-.15-3.09-.38-4.55H24v9.02h12.94c-.58 2.96-2.26 5.48-4.78 7.18l7.73 6c4.51-4.18 7.09-10.36 7.09-17.65z"/>
            <path fill="#FBBC05" d="M10.53 28.59c-.48-1.45-.76-2.99-.76-4.59s.27-3.14.76-4.59l-7.98-6.19C.92 16.46 0 20.12 0 24c0 3.88.92 7.54 2.56 10.78l7.97-6.19z"/>
            <path fill="#34A853" d="M24 48c6.48 0 11.93-2.13 15.89-5.81l-7.73-6c-2.15 1.45-4.92 2.3-8.16 2.3-6.26 0-11.57-4.22-13.47-9.91l-7.98 6.19C6.51 42.62 14.62 48 24 48z"/>
          </svg>
          Đăng nhập bằng Google
        </button>
        
        <p class="login-footer">
          Đăng nhập bằng tài khoản Google đã được quản trị viên cấp quyền.<br>
          Người đăng nhập đầu tiên sẽ trở thành quản trị viên.
        </p>
      </div>
    </div>
  `;

  // Event: Google login
  document.getElementById('btnGoogleLogin').addEventListener('click', async () => {
    const btn = document.getElementById('btnGoogleLogin');
    btn.disabled = true;
    btn.innerHTML = `
      <div class="loading-spinner" style="width:20px;height:20px;">
        <div class="spinner-ring" style="border-width:2px;"></div>
      </div>
      Đang đăng nhập...
    `;

    try {
      await signInWithGoogle();
      // Auth state change will handle redirect
    } catch (err) {
      if (err.message && err.message.startsWith('UNAUTHORIZED')) {
        const emailInfo = err.message.split(':')[1] || '';
        showToast({
          message: 'Tài khoản ' + (emailInfo ? `(${emailInfo}) ` : '') + 'chưa được cấp quyền. Liên hệ quản trị viên để thêm chính xác email này vào hệ thống.',
          type: 'error',
          duration: 6000
        });
      } else if (err.code !== 'auth/popup-closed-by-user') {
        showToast({ message: 'Lỗi đăng nhập: ' + err.message, type: 'error' });
      }
      
      btn.disabled = false;
      btn.innerHTML = `
        <svg width="20" height="20" viewBox="0 0 48 48">
          <path fill="#EA4335" d="M24 9.5c3.54 0 6.71 1.22 9.21 3.6l6.85-6.85C35.9 2.38 30.47 0 24 0 14.62 0 6.51 5.38 2.56 13.22l7.98 6.19C12.43 13.72 17.74 9.5 24 9.5z"/>
          <path fill="#4285F4" d="M46.98 24.55c0-1.57-.15-3.09-.38-4.55H24v9.02h12.94c-.58 2.96-2.26 5.48-4.78 7.18l7.73 6c4.51-4.18 7.09-10.36 7.09-17.65z"/>
          <path fill="#FBBC05" d="M10.53 28.59c-.48-1.45-.76-2.99-.76-4.59s.27-3.14.76-4.59l-7.98-6.19C.92 16.46 0 20.12 0 24c0 3.88.92 7.54 2.56 10.78l7.97-6.19z"/>
          <path fill="#34A853" d="M24 48c6.48 0 11.93-2.13 15.89-5.81l-7.73-6c-2.15 1.45-4.92 2.3-8.16 2.3-6.26 0-11.57-4.22-13.47-9.91l-7.98 6.19C6.51 42.62 14.62 48 24 48z"/>
        </svg>
        Đăng nhập bằng Google
      `;
    }
  });
}
